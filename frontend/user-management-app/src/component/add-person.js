...
function AddPerson(){
  ...
  const handleSubmit = (event) => {
    ...
    if(isSubmit){
      axios.post("http://localhost:8080/api/person", personDetails,{
        headers:{
      ...
      (response) => {
        const errors = {};
        if(response.data.message==="Person Saved successfully"){
          Swal.fire("Success!", "Person saved successfully!", "success");
          navigate("/dashboard")
        }
        else{
